...
- 更新全局变量
- 总欠款数量为`borrower.borrowBalance * market.borrowIndex / borrower.borrowIndex`
...
3. 在区块高度200处，我们来计算所有的利息；假设在此期间也并没有发生任何借款/还款/存款/赎回等操作。
   ...
   用户B在此时要偿还债务，他欠款总额为`欠款总额 * 全局borrowIndex / 借款时的borrowIndex `，按照公式套入计算为`50 * 49.5 / 8.25 = 300`。也就是说B将偿还`300eth`才能还清债务。

    用户A在此时选择赎回，此时的兑换率为`(50 + 300 - 25)/50=6.5`; 那么A将获得`50xeth * 6.5= 325eth`

    

   然后我们再来看看结论，在B还清债务后，池子里总共有`50 + 300 = 350`个eth，A赎回时拿走`325`个`eth`; `xeth`完全销毁，池子里剩余`350 - 325 = 25`eth, 刚好就是`totalReserves`的数量。完全吻合。
